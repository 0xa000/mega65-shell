## mega65-shell — QMTECH Wukong DFX: child-configuration constraints
##
...
## static.xdc is not possible (pin constraints on a locked netlist make
## the placer try to modify DONT_TOUCH static logic, e.g. IDELAYCTRL
## replication). Keep in sync with the corresponding static.xdc rules.
...
## QNICE-framework constraints (sdcard clock divider, EAE multicycles)
## live in the RM framework's own XDC (e.g. qnice-rm.xdc in the M2M fork) —
## framework-based children read that file in addition to this one.
...
## FF-oriented "..._reg.*/C" rules above do not match (same lesson as the
## flat WUKONG.xdc).  Full-match "/CLK" only — inert if the FIFOs map to
## BRAM (clock pins CLKARDCLK/CLKBWRCLK).  First RM to need this: C64
## (higher LUT pressure than VIC20/democore).
...
## data crossing goes through the framework's dual-clock RAMs / CDC synchronisers
## (e.g. the c1541 disk buffer feeding the gcr logic).  The flat build gets this
## for free because loader_clk and the core sit on separate, unrelated MMCM trees; but
## a child impl re-derives the core clocks from sys_clk_100 (the per-RM clock
## override), which makes them share loader_clk's clk_100 ancestor and time
## synchronously with a near-zero requirement.  Restore CDC treatment.  Covers
## both mux sources (core_clk0/core_clk0_b, core_clk1/core_clk1_b); -quiet so it
## is inert for an RM that leaves a core clock unused.
