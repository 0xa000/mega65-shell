...
--
-- digital_pipeline_rm: digital_pipeline with only the OSERDES serialisers
-- removed (boundary v1). The video seam of the DFX boundary is the TMDS
-- word stream: vga_to_hdmi (TMDS encode, InfoFrames, audio data islands)
-- and the PCM rate strobes live HERE in the RM; only the serialisers and
-- the pixel-clock MMCM are in the static shell.
-- See m65-shell-poc/docs/BOUNDARY-V1.md.
--
-- Generated as a fork of digital_pipeline.vhd; keep diffs against it reviewable.
...
end entity digital_pipeline_rm;

architecture synthesis of digital_pipeline_rm is
...
begin
   ...
   -- DFX boundary v1: hand the encoded TMDS words to the static shell,
   -- which owns only the OSERDES serialisers (and the pixel-clock MMCM).
   tmds_o <= hdmi_tmds;
