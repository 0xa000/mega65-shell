...
--
-- av_pipeline_rm: av_pipeline with only the OSERDES serialisers removed
-- (boundary v1). The DFX video seam is the TMDS word stream out of
-- vga_to_hdmi (which lives RM-side, inside digital_pipeline_rm, together
-- with the PCM rate strobes); the shell owns only the serialisers and the
-- pixel-clock MMCM. See m65-shell-poc/docs/BOUNDARY-V1.md.
--
-- Generated as a fork of av_pipeline.vhd; keep diffs against it reviewable.
...
end entity av_pipeline_rm;

architecture synthesis of av_pipeline_rm is
...
begin
   ...
   i_digital_pipeline : entity work.digital_pipeline_rm
      generic map (
      ...
      port map (
         ...
         -- Encoded TMDS words towards the shell (DFX boundary v1)
