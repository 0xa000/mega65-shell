## MiSTer2MEGA65 — QMTECH Wukong DFX: child-configuration constraints
##
...
## WUKONG-DFX.xdc is not possible (pin constraints on a locked netlist make
## the placer try to modify DONT_TOUCH static logic, e.g. IDELAYCTRL
## replication). Keep in sync with the corresponding WUKONG-DFX.xdc rules.
...
## live in qnice-rm.xdc — QNICE-based children read that file in addition
## to this one; non-QNICE RMs (picorv32 menu) read only this file.
...
## for free because qnice and the core sit on separate, unrelated MMCM trees; but
## a child impl re-derives the core clocks from sys_clk_100 (the per-RM clock
## override), which makes them share qnice's clk_100 ancestor and time
