# MiSTer2MEGA65 — QMTECH Wukong RM build (mega65-shell SDK overlay)
#
...
#        RM_XDC=".../wukong-dfx-child.xdc .../qnice-rm.xdc"
#
# Usage (run from CORE/):
#   vivado -mode batch -source wukong-dfx-build.tcl -tclargs rm_elab   # RM-only elaboration sanity check
#   vivado -mode batch -source wukong-dfx-build.tcl -tclargs rm_synth  # RM out-of-context synthesis -> rm_democore_synth.dcp
#
# Trimmed from the full DFX PoC flow (MiSTer2MEGA65 fork, branch dfx-v5);
...
# ---------------------------------------------------------------------------
# RM sources: the flat wukong-build.tcl list minus everything the shell owns
# (clk_wukong, clk_m2m, video_out_clock, CORE clk.vhd, ddr3 wrapper + bridge
# + UberDDR3, OSERDES serialisers, board top) plus the dfx/ RM variants
# (av_pipeline_rm/digital_pipeline_rm shadow their originals; mega65_rm
# shadows CORE/vhdl/mega65.vhd under the same entity name MEGA65_Core)
# ---------------------------------------------------------------------------
...
set rm_vhdl_2008 {
   ...
   ../M2M/vhdl/wukong/dfx/digital_pipeline_rm.vhd
   ../M2M/vhdl/wukong/dfx/av_pipeline_rm.vhd
   ../M2M/vhdl/wukong/dfx/framework_rm.vhd
   vhdl/dfx/democore_clk_pkg.vhd
   ../M2M/vhdl/wukong/dfx/clk_drp_master.vhd
   ../M2M/vhdl/wukong/dfx/rm_top.vhd
}
...
# Single democore RM (out-of-context).  It reprograms CORE_A to 54 MHz at wake
# via the DRP proxy (democore's native clock), so swapping back to it from any
# other RM restores the right frequency instead of inheriting the previous RM's
# — the "RM clocking rule".
if { $stage == "rm_synth" } {
   read_rm_sources
