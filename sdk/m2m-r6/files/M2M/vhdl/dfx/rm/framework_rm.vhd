...
--
-- framework_rm: framework.vhd with everything the static shell owns removed.
-- Clock generation (clk_m2m, video_out_clock), the reset manager, the
-- HyperRAM controller and the HDMI serialisers live in the shell; this
-- module receives all clocks/resets as ports, drives one arbitrated
-- Avalon-MM master towards the shell's memory port, hands encoded TMDS
-- words plus VGA/VDAC and processed PCM across the boundary, and exposes
-- the I2C buses as in/out pairs (the open-collector tri-states are
-- shell-side IOBs).
-- See m65-shell-poc/docs/BOUNDARY-R6.md. Fork of framework.vhd; keep diffs
-- against it reviewable.
--
...
port (
   ...
   hr_clk_i                : in    std_logic;                  -- 100 MHz (shell's HyperRAM/Avalon domain)
   ...
   -- (DFX boundary: only serialisers + pixel-clock MMCM are static)
   ...
   -- (DFX boundary; @ hr_clk_i; the HyperRAM controller lives in the shell)
   ...
   -- I2C buses as logical in/out pairs across the DFX boundary; the
   -- open-collector tri-states ('0' when out='0' else 'Z') are shell-side
   -- IOBs.
...
-- diff against framework.vhd small
...
begin
   ...
   ---------------------------------------------------------------------------------------------------------------
   -- External memory: the HyperRAM controller lives in the static shell —
   -- hand the arbitrated Avalon-MM master across the DFX boundary
   ---------------------------------------------------------------------------------------------------------------
   ...
   -- The controller's access-statistics counters stayed shell-side and are
   -- not part of the boundary; the firmware statistics page reads zeros
   ...
   port map (
      ...
      scl_in_i      => "11" & audio_scl_in_i & vga_scl_in_i & hdmi_scl_in_i & i2c_scl_in_i & grove_scl_in_i & fpga_scl_in_i,
      sda_in_i      => "11" & audio_sda_in_i & vga_sda_in_i & hdmi_sda_in_i & i2c_sda_in_i & grove_sda_in_i & fpga_sda_in_i,
      scl_out_o     => scl_out,
      sda_out_o     => sda_out
   ); -- i_rtc_wrapper

   -- The open-collector tri-states ('0' when out='0' else 'Z') are IOBs and
   -- therefore live in the static shell; only the logical levels cross the
   -- DFX boundary
   fpga_sda_out_o  <= sda_out(0);
   fpga_scl_out_o  <= scl_out(0);
   grove_sda_out_o <= sda_out(1);
   grove_scl_out_o <= scl_out(1);
   i2c_sda_out_o   <= sda_out(2);
   i2c_scl_out_o   <= scl_out(2);
   hdmi_sda_out_o  <= sda_out(3);
   hdmi_scl_out_o  <= scl_out(3);
   vga_sda_out_o   <= sda_out(4);
   vga_scl_out_o   <= scl_out(4);
   audio_sda_out_o <= sda_out(5);
   audio_scl_out_o <= scl_out(5);
