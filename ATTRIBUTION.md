...
|---|---|---|---|
| `rtl/common/` framework helpers: `cdc_stable.vhd`, `reset_manager.vhd`, `axi_fifo_small.vhd`, `avm_increase.vhd`, `shell_clk_base.vhd` (from `clk_m2m.vhd`) | [MiSTer2MEGA65](https://github.com/sy2002/MiSTer2MEGA65) framework, by sy2002 and MJoergen | GPL-3.0 | `avm_increase.vhd` carries two local fixes (RESPONSE_ST stale-word, abort-drain fencing). `shell_clk_base.vhd` is `clk_m2m.vhd` with shell-neutral clock/port names (`qnice_*` → `loader_*`, `hr_*` → `mem_*`). `reset_manager.vhd`: `reset_m2m_n_o` → `reset_shell_n_o`. |
| `rtl/common/types_pkg.vhd`, `rtl/common/video_out_clock.vhd`, `rtl/common/serialiser_10to1_selectio.vhd` | The Tyto Project, (C) Adam Barnes, as distributed (with modifications by MJoergen) in MiSTer2MEGA65 | LGPL-3.0-or-later | Common types package, dynamically reconfigured pixel-clock MMCM, 10:1 OSERDES serialiser. |
| `rtl/common/debounce.vhd` | Digi-Key eewiki, by Scott Larson | Provided "AS IS" (public release, no copyleft) | Button debouncer; imported via MiSTer2MEGA65. |
