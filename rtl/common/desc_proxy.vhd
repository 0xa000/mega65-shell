...
-- data[3:0] partition: 0 auto / 1..4 MBR slot), 1..4 START (MSB first),
-- 5..8 LENGTH (MSB first), 9 GO.
--
-- GO with the walker busy is ignored (RM can poll bit 9). GO with an
-- invalid mode/partition sets the error latch to 0x8F without firing.
-- The error latch clears on the next GO and on decouple.
--
...
begin
  ...
  rsv_to_rm <= ack & "00000" & wk_busy & err_latch & diag_r;
