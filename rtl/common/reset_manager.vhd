...
-- b) Whole system (shell included): Duration defined by SHELL_RST_TRIGGER (see below)
--
...
-- Press the MEGA65's reset button long to activate the whole-system (shell) reset, press it short for a core-only reset
constant SHELL_RST_TRIGGER      : natural := 1500;   -- milliseconds => 1.5 sec
      ...
      if rising_edge(CLK) then
         ...
         if dbnce_reset_n = '0' then
            ...
            if button_duration = 0 then
               reset_shell_n_o     <= '0';  -- the shell only resets if the trigger time is reached
            else
         ...
         -- button released
            ...
            if reset_pressed then
               ...
               reset_shell_n_o     <= '1';
               reset_core_n_o    <= '1';
               button_duration   <= (BOARD_CLK_SPEED / 1000) * SHELL_RST_TRIGGER;
            end if;
         end if;
      end if;
